...
def get_assoc_table(ip, base_oid):
    ...
    print(f"   [PARSED] {len(data)} client(s) found: {list(data.keys())}", flush=True)
    return data

def wait_for_association(ip, timeout=60):
    print(f"Waiting up to {timeout}s for client association...", flush=True)
    start = time.time()
    while time.time() - start < timeout:
        ...
        if output and "No Such Object" not in output and len(output.strip()) > 0:
            print(f"Client associated after {int(time.time()-start)}s!", flush=True)
            return True
        print(f"   Still waiting... ({int(time.time()-start)}s)", flush=True)
        time.sleep(5)
    print(f"Timeout: No client associated after {timeout}s", flush=True)
    return False
...
def test_roaming_snmp(remote_ip, iter):
    ...
    try:
        ...
        if not wait_for_association(su_ip, timeout=60):
            result["Device Logs"] += "BSU1: No client associated after 60s\n"
        else:
        ...
        if not wait_for_association(su_ip, timeout=60):
            result["Device Logs"] += "BSU2: No client associated after 60s"
        else:
            result["Assoc Table BSU2"] = get_assoc_table(su_ip, OID_ASSOC_BASE)
